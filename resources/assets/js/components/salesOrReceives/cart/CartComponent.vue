            ...
            <div v-else class="cart-item-container py-1" v-for="(cartItem,index) in cart"
                ...
                <div class="form-row mx-0 px-2  collapse-animation"
                    ...
                    <div class="form-group pr-0 mb-zero" :class="[checkDiscount() ? 'col-4':'col-6']"
                     ...
                     <!-- <div class="form-group pl-0 mb-zero" :class="[checkDiscount() ? 'col-3':'col-6']">
                        <label :for="'cart-item-room-length'+index" class="label-in-cart ">
                            Area Length</label>
                        <input type="text" :id="'cart-item-room-length'+index" class="form-control" v-model="cartItem.areaLength">
                    </div>
                    <div class="form-group pl-0 mb-zero" :class="[checkDiscount() ? 'col-3':'col-6']">
                        <label :for="'cart-item-room-width'+index" class="label-in-cart ">
                            Area Width</label>
                        <input type="text" :id="'cart-item-room-width'+index" class="form-control" v-model="cartItem.areaWidth">
                    </div>
                    <div class="form-group pl-0 mb-zero" :class="[checkDiscount() ? 'col-6':'col-6']">
                      <label :for="'cart-item-room-length'+index" class="label-in-cart">&nbsp;</label>
                        <button  href="#" class="btn pay-btn app-color" @click="calculateQuantityOfArea(cartItem, index)">Calculate Quantity</button>
                    </div>

                    <div class="form-group pl-0 mb-zero" :class="[checkDiscount() ? 'col-3':'col-3']">
                        <label :for="'cart-item-area-type'+index" class="label-in-cart ">
                            Area Type</label>
                      <select :id="'cart-item-area-type'" class="custom-select">
                        <option value="r1"><span>Room1</span></option>
                        <option value="r2"><span>Room2</span></option>
                      </select>
<!--                        <input type="text" :id="'cart-item-area-type'+index" class="form-control" v-model="cartItem.areaType">-->
                    <!-- </div>
                    <div class="form-group pl-0 mb-zero" :class="[checkDiscount() ? 'col-3':'col-3']">
                          <label :for="'cart-item-area-type'+index" class="label-in-cart ">
                              Area Type</label>
                    ...
                    <div class="form-group pl-0 mb-zero" :class="[checkDiscount() ? 'col-3':'col-3']">
                        <label class="label-in-cart">&nbsp;</label>
                        <input type="file" :id="'cart-item-room-fp'+index" class="" accept="image/*"  @change="event => floorPlanImage(event, index, cartItem)">
                    </div> -->
...
<!--                    </div>-->
        ...
        <div id="cart-section-3" class="position-absolute fixed-bottom product-card-font">
            <div class="row mx-0 px-3 py-2 font-weight-bold border-top border-bottom">
                <div class="cl-md-12">
                    <textarea rows="2" class="form-control" style="height: 200px"></textarea>
                </div>
            </div>
            <div class="row mx-0 px-3 py-2 font-weight-bold border-top border-bottom">
                <div class="col-6 p-0">
...
export default {
    ...
    methods: {
      ...
      // floorPlanImage(event, index, cartItem) {
      //   let fileName = event.target.files[0].name;
      //   $("#cart-item-room-fp"+index)
      ...
      //     reader.onload = e => {
      //       cartItem[index].floorPlan = e.target.result;
      //     };
      //     reader.readAsDataURL(input.files[0]);
      //   } else {
      //     cartItem[index].floorPlan = "";
      //   }
      // },
      ...
      floorPlanImage(event, index, cartItem) {
        ...
        if (input.files && input.files[0]) {
          let reader = new FileReader();
          reader.onload = (e) => {
            cartItem.floorPlan = e.target.result;
          };
          reader.readAsDataURL(input.files[0]);
        } else {
          cartItem.floorPlan = "";
      ...
      calculateQuantityOfArea(cartItem, index) {

          const areaWidth = parseInt(cartItem.areaWidth);
          const areaLength = parseInt(cartItem.areaLength);

          if ( (!isNaN(areaWidth) && areaWidth > 0 && cartItem.areaWidth === areaWidth.toString()) && (!isNaN(areaLength) && areaLength > 0 && cartItem.areaLength === areaLength.toString()) ){
            const qty = areaLength*areaWidth;
            if(qty > cartItem.availbleQuantity) {
              alert("You can't add quantity more than available quantity");
              cartItem.quantity = 1;
              cartItem.areaWidth = '';
              cartItem.areaLength = '';
            } else {
              cartItem.quantity = 0;
              cartItem.quantity = areaLength*areaWidth;
            }
          }
        }
