...
class SalesController extends Controller
{
    ...
    public function salesView()
    {
        ...
        $output = [
            ...
            'defaultInvoiceTemplateForSales' => view('invoiceTemplates.defaultSalesThermal')->render(),
...
//        $areaLength = $request->areaLength;
        ...
        if (($orderStatus == 'done' && !$orderID) || ($orderStatus == 'pending' && !$orderID) || ($orderStatus == 'hold' && !$orderID)) {
            ...
            $orderData['custom_payment_type'] = $request->custom_payment_type??'';
        ...
        foreach ($carts as $cart) {
            ...
            if($orderType == 'sales') {
                $floorPlan = '';
                if ($file = $floorPlan) {
                    $fileNameToStore = $imageHandler->imageUpload($floorplan, 'floorplan', 'uploads/floorplan/');
                }else{
                    $fileNameToStore = '';
                }
                $areaLength=$car['areaLength']??'';
                $areaWidth=$car['areaWidth']??'';
                $areaType=$car['areaType']??'';
            }else{
                $areaLength='';
                $areaWidth='';
                $areaType='';
    ...
    public function sendPdf($templateData, $orderID, $cashRegister, $mailText, $email, $subject)
    {
        try {
            ...
            $fileNameToStore = 'The Carpet Centre - ' . $order->invoice_id . '.pdf';
