...
class ProductsController extends Controller
{
    ...
    public function storeProduct(Request $request)
    {
        ...
        $comparePrice1 = $request->comparePrice1??'';
        $comparePrice2 = $request->comparePrice2??'';
        ...
        $productData = [
            ...
            'compare_price1' => $comparePrice1,
            'compare_price2' => $comparePrice2,
            'supplier' => $supplier
        ];
        if ($taxID == 'no-tax') {
    ...
    public function editProduct(Request $request, $id)
    {
        ...
        $data['compare_price1'] = $request->comparePrice1??'';
        $data['compare_price2'] = $request->comparePrice2??'';
    ...
    {
        //check if product have zero quantity then delete the product

        $quantity = (int) OrderItems::where('product_id', $id)->sum('quantity');

        $getProductSell = (int) OrderItems::where('product_id', $id)->where('type', 'sales')->get()->count();
        
        if (OrderItems::checkExists('product_id', $id)  && $getProductSell !== 0) {
            $response = [
