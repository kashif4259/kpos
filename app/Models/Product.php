...
class Product extends BaseModel
{
    ...
    protected $fillable = ['title', 'description', 'category_id', 'brand_id', 'unit_id', 'group_id', 'taxable', 'tax_type', 'tax_id', 'product_type', 'branch_id', 'imageURL', 'created_by', 'product_length', 'product_width', 'compare_price1', 'compare_price2', 'supplier'];
    ...
    public function getProductReportsData($offset = 0, $limit = 10, $column = "", $order = "", $search = "", $startDate = "", $endDate = "")
    {
        $query = Product::query()
            ->leftJoin('product_variants', 'product_variants.product_id', '=', 'products.id')
            ->select(
    ...
    public function getProductReportInventory($page = 0, $perPage = 10, $column = "", $order = "", $search = "", $startDate = "", $endDate = "")
    {
        $query = DB::table('products')
            ->selectRaw(" 
                    products.id,
                    products.title,
                    SUM(order_items.quantity) as available,
                    (SELECT SUM(order_items.quantity) FROM order_items WHERE product_variants.product_id = order_items.product_id AND order_items.`type` = 'receiving')   as total,
	                (SELECT SUM(order_items.quantity) FROM order_items WHERE product_variants.product_id = order_items.product_id AND order_items.`type` = 'sales')   * -1 as sold
                    ")
            ->leftJoin("product_variants", "products.id", "=", "product_variants.product_id")
            ->leftJoin("order_items", "products.id", "=", "order_items.product_id")
            // ->where("products.id",">=",150)
    ...
    public function getProductReportProfitLoss($page = 0, $perPage = 10, $column = "", $order = "", $search = "", $startDate = "", $endDate = "")
    {
        $profitLossQ = "(SUM(order_items.sub_total) - SUM(product_variants.purchase_price * (order_items.quantity * -1)))";
        $query = DB::table('products')
            ->selectRaw(" 
                    products.id,
                    products.title,
                    SUM(order_items.quantity) * -1 as quantity,
                    SUM(product_variants.purchase_price) as purchase_total,
                    SUM(order_items.sub_total) as sold_total,
                    IF( $profitLossQ > 0,$profitLossQ,'-') as 'profit',
                    IF( $profitLossQ < 0,$profitLossQ,'-') as 'loss'
                    ")
            ->leftJoin("product_variants", "products.id", "=", "product_variants.product_id")
            ->leftJoin("order_items", "products.id", "=", "order_items.product_id")
